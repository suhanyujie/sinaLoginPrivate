...
class Main 
{
	protected $username = '';
	protected $passwd = '';
    public $testUrl = 'http://d.weibo.com/p/aj/v6/mblog/mbloglist?ajwvr=6&domain=102803_ctg1_2088_-_ctg1_2088&from=faxian_hot&mod=fenlei&pagebar=0&tab=home&current_page=1&pre_page=1&page=1&pl_name=Pl_Core_NewMixFeed__3&id=102803_ctg1_2088_-_ctg1_2088&script_uri=/102803_ctg1_2088_-_ctg1_2088&feed_type=1&domain_op=102803_ctg1_2088_-_ctg1_2088&__rnd=1490174458161';

	protected $configPath = '/www/2017/sinaLoginPrivate/wbcookie/config.inc';
    public static $configPathStatic = '/www/2017/sinaLoginPrivate/wbcookie/config.inc';
    ...
    public function setConfig()
    {
    	...
    	if(!file_exists($savePath))throw new Exception("there is no this file[无法设置授权信息].in class".__CLASS__);
    	if($this->username && $this->passwd){
            $bytes = file_put_contents($savePath, json_encode($content));
        }
    }
    ...
    public function getPreParam(){
    	try {
    		$getCookie = new GetWeiboCookie($this->configPath);
    		//获得配置
    		$getCookie->getConfig();
    		//获得预登录参数，跳转到预登陆地址
    		$getCookie->getPreUrl();
...
//     		abort(504,$e->getMessage());
    ...
    {   
        ...
        if(!file_exists($savePath)){
            ...
            $configInfo = file_get_contents($savePath);
        }
        
        $preParam = json_decode($configInfo, true);
...
//          document.write(encrpt);
            window.location.href='/2017/sinaLoginPrivate/index.php?a=browserLogin&sp='+encrpt;  
    </script>";
            return ;
        }
    }
    ...
    public function browserLogin()
    {
        ...
        if($preParam['showpin'] == 1){          
            ...
            echo '有验证码啦：<img src="'.$preParam['doorImg'].'">';exit();
        } else {
            // 如果没有图片验证码
            $preParam ['door'] = '';
            if ($this->getCookiePack($preParam)) {
                header("Location:/2017/sinaLoginPrivate/index.php?a=setTestUrl&test_url=".$this->testUrl);
    ...
    public function getCookie()
    {
        ...
        if ($this->getCookiePack($preParam)) {
            // return Redirect ( "admin/authorize/seccuss" );
            throw new Exception('Redirect ( "admin/authorize/seccuss" );', 1);
            
        } else {
            // return Redirect ( "admin/authorize/fail" );
            throw new Exception('Redirect ( "admin/authorize/fail" )', 1);
            
        }
    }


    public function getTestContent($requestUrl)
    {
        $url = $requestUrl;
        //微博cookie
        ...
        if(!file_exists($cookieWeibo)){
            throw new Exception("未获得授权信息，请重新登录", 1);
